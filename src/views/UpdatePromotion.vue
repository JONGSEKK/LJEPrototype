<script setup>
    ...
    onMounted(async function (){
        namaP.value=mosi.value.namaPromosi;
        hargaP.value=mosi.value.hargaPromosi;
        deskripsiP.value=mosi.value.deskripsiPromosi;
        kategori.value=mosi.value.kategori;
    })
    ...
    async function modify(){
        ...
        const record = await pb.collection('promosi').update(mosi.value.id, data);

        mosi.value.namaPromosi=namaP.value;
        mosi.value.hargaPromosi=hargaP.value;
        mosi.value.deskripsiPromosi=deskripsiP.value;
        mosi.value.kategori=kategori.value;

        alert("Berjaya Ubah Suai Maklumat Promosi!");
        redirectHomepage();
    }
...
<template>
    ...
    <div class="bg-blue-100 min-h-screen pl-64 max-md:pl-0">
        <h1 class="text-3xl font-semibold pb-14 text-bl pt-6 pl-5 ">Ubah Suai Maklumat Promosi</h1>
        <div class="bg-white w-11/12 m-auto rounded-2xl shadow-2xl">
            <form class="w-3/5 m-auto py-14" v-on:submit.prevent="modify()">
