...
async function daftarProduk(){
    ...
    "kuantiti": kuantitiI.value,
    };

    const record = await pb.collection('inventori').create(data);
    console.log(record);
...
<template>
    ...
    <div class="bg-blue-100 min-h-screen pl-64 max-md:pl-0">
        <h1 class="text-3xl font-semibold pb-14 text-bl pt-6 pl-5 ">Tambah Produk</h1>
        <div class="bg-white w-11/12 m-auto rounded-2xl shadow-2xl">
            <form class="w-3/5 m-auto py-14" v-on:submit.prevent="daftarProduk">
                <h2 class="text-l font-medium pb-7 text-gray-600">Sila Masukkan Maklumat Produk Dalam Ruangan Yang Disediakan</h2>
                        <table class="w-full">
                            ...
                            <tr>
                                <td class="w-44 py-5"><MyLabel label="Kuantiti"/></td>
                                <td><input type="text" placeholder="Kuantiti Produk" v-model="kuantitiI" class="w-full outline-none border-b-2 border-b-blue-600 bg-slate-50 drop-shadow-md"/></td>
                            </tr>
                            <tr>
                                <td colspan="2">
